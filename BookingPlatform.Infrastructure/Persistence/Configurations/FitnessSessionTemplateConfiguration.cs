using BookingPlatform.Domain.Fitness;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Metadata.Builders;

namespace BookingPlatform.Infrastructure.Persistence.Configurations;

public sealed class FitnessSessionTemplateConfiguration : IEntityTypeConfiguration<FitnessSessionTemplate>
{
    public void Configure(EntityTypeBuilder<FitnessSessionTemplate> builder)
    {
        builder.ToTable("fitness_session_templates");

        builder.HasKey(x => x.Id);

        builder.Property(x => x.SessionType)
            .HasConversion<int>()
            .IsRequired();

        builder.Property(x => x.DayOfWeek)
            .IsRequired();

        builder.Property(x => x.StartTime)
            .IsRequired();

        builder.Property(x => x.DurationMin)
            .HasDefaultValue(60)
            .IsRequired();

        builder.Property(x => x.Capacity)
            .IsRequired();

        builder.Property(x => x.IsActive)
            .HasDefaultValue(true)
            .IsRequired();

        builder.Property(x => x.ValidFromDate);

        builder.Property(x => x.ValidToDate);

        builder.Property(x => x.Note)
            .HasMaxLength(1000);

        builder.HasIndex(x => x.BusinessId);
        builder.HasIndex(x => x.FitnessRoomId);
        builder.HasIndex(x => x.FitnessClassTypeId);
        builder.HasIndex(x => x.TrainerStaffMemberId);

        builder.HasIndex(x => new
        {
            x.FitnessRoomId,
            x.DayOfWeek,
            x.StartTime
        });

        builder.HasOne(x => x.Business)
            .WithMany()
            .HasForeignKey(x => x.BusinessId)
            .OnDelete(DeleteBehavior.Cascade);

        builder.HasOne(x => x.FitnessRoom)
            .WithMany(x => x.SessionTemplates)
            .HasForeignKey(x => x.FitnessRoomId)
            .OnDelete(DeleteBehavior.Cascade);

        builder.HasOne(x => x.FitnessClassType)
            .WithMany(x => x.SessionTemplates)
            .HasForeignKey(x => x.FitnessClassTypeId)
            .OnDelete(DeleteBehavior.SetNull);

        builder.HasOne(x => x.TrainerStaffMember)
            .WithMany()
            .HasForeignKey(x => x.TrainerStaffMemberId)
            .OnDelete(DeleteBehavior.SetNull);
    }
}
